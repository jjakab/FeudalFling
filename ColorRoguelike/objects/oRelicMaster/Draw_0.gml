...
draw_text_ext_transformed(xLoc,yHeight + 3 - (sprite_get_height(sShopCaptionMiddle) * yScaleFactor) - sprite_get_height(sShopCaptionBottom),relicTitle,sep,width,.25,.25,0)
...
draw_text_ext_transformed(xLoc - sprite_get_width(sShopCaptionBottom) / 4,yHeight,rarity,sep,width,0.25,0.25,0)

//Draw the gold cost

draw_set_color(c_white)
var goldWidth = string_width_ext(string(goldCost),sep,width) / 4
draw_text_ext_transformed(xLoc - (goldWidth / 4) + sprite_get_width(sShopCaptionBottom) / 4,yHeight,goldCost,sep,width,0.25,0.25,0)
